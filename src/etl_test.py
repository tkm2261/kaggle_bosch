...
def etl(train_data, num, feature_column, date_cols):
    ...
    for i in range(52):
        ...
        train_data['L%s_S%s_NUM_DIFF' % (line, i)] = train_data['L%s_S%s_NUM_MAX' %
                                                                (line, i)] - train_data['L%s_S%s_NUM_MIN' % (line, i)]
...
if __name__ == '__main__':
    ...
    for i in range(52):
        ...
        if len(cols) == 0:
            continue
        line = cols[0][1]
        col_names = ['L%s_S%s_NUM_MAX' % (line, i), 'L%s_S%s_NUM_MIN' % (
            line, i), 'L%s_S%s_NUM_AVG' % (line, i), 'L%s_S%s_NUM_DIFF' % (line, i)]
        feature_column += col_names
    ...
    for train_data in train_data_all:
        aaa = train_data[LIST_FEATURE_COLUMN_NAME].apply(lambda row: hashlib.sha1(
            (','.join(map(str, row))).encode('utf-8')).hexdigest(), axis=1)
        train_data['__hash__'] = aaa
        train_data = pandas.merge(train_data, mst, how='left', left_on='__hash__', right_index=True,  copy=False)
        aaa = train_data[LIST_COLUMN_NUM].apply(lambda row: hashlib.sha1(
            (','.join(map(str, row))).encode('utf-8')).hexdigest(), axis=1)
        ...
        aaa = train_data[LIST_COLUMN_CAT].apply(lambda row: hashlib.sha1(
            (','.join(map(str, row))).encode('utf-8')).hexdigest(), axis=1)
        ...
        aaa = train_data[LIST_COLUMN_DATE].apply(lambda row: hashlib.sha1(
            (','.join(map(str, row))).encode('utf-8')).hexdigest(), axis=1)
        ...
        for i in range(4):
            ...
            aaa = train_data[cols].apply(lambda row: hashlib.sha1(
                (','.join(map(str, row))).encode('utf-8')).hexdigest(), axis=1)
            train_data['__hash_%s__' % i] = aaa
            train_data = pandas.merge(train_data, list_mst[i]['mst'], how='left',
                                      left_on='__hash_%s__' % i, right_index=True,  copy=False)

            cols2 = [col for col in cols if col in LIST_COLUMN_NUM]
            aaa = train_data[cols2].apply(lambda row: hashlib.sha1(
                (','.join(map(str, row))).encode('utf-8')).hexdigest(), axis=1)
            train_data['__hash1_%s__' % i] = aaa
            train_data = pandas.merge(train_data, list_mst[i]['num'], how='left',
                                      left_on='__hash1_%s__' % i, right_index=True,  copy=False)

            cols2 = [col for col in cols if col in LIST_COLUMN_CAT]
            aaa = train_data[cols].apply(lambda row: hashlib.sha1(
                (','.join(map(str, row))).encode('utf-8')).hexdigest(), axis=1)
            train_data['__hash2_%s__' % i] = aaa
            train_data = pandas.merge(train_data, list_mst[i]['cat'], how='left',
                                      left_on='__hash2_%s__' % i, right_index=True,  copy=False)

            cols2 = [col for col in cols if col in LIST_COLUMN_DATE]
            aaa = train_data[cols2].apply(lambda row: hashlib.sha1(
                (','.join(map(str, row))).encode('utf-8')).hexdigest(), axis=1)
            train_data['__hash3_%s__' % i] = aaa
            train_data = pandas.merge(train_data, list_mst[i]['date'], how='left',
                                      left_on='__hash3_%s__' % i, right_index=True,  copy=False)
        ...
        for i in range(52):
            ...
            train_data = pandas.merge(train_data, map_sec_mst[line, i], how='left', left_on='__hash_%s_%s__' % (
                line, i), right_index=True,  copy=False)
