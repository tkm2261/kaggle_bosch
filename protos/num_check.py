...
if __name__ == '__main__':
    ...
    feature_column = [col for col in feature_column if col in LIST_COLUMN_NUM]

    for i in list(range(4)) + ['']:
        cols = [col for col in feature_column if 'L%s' % i in col]
        train_data['L%s_NUM_MAX' % i] = train_data[cols].max(axis=1)
        train_data['L%s_NUM_MIN' % i] = train_data[cols].min(axis=1)
        train_data['L%s_NUM_AVG' % i] = train_data[cols].mean(axis=1)
    ...
    df_fi.to_csv('feature_importances_num.csv', index=False)
